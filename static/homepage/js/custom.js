...
login_check.open("GET", "/api/account/login_check/", true);
login_check.withCredentials = true;
login_check.send();
login_check.onload = function () {
    if (login_check.status === 200) {
        let user_data = JSON.parse(login_check.responseText);
        $("#profile_avatar").attr("src", user_data["user"]["icon"]);
        $("#register_link").css("display", "none");
        $("#logout_link").css("display", "block");
        $("#dashboard_link").css("display", "block");
        $("#profile_name").text(user_data["user"]["username"]);
    }
    else{
        $("#logout_link").css("display", 'none')
        $("#dashboard_link").css("display", 'none')
        $("#register_link").css("display", 'block')
    }
}
$("#logout_link").on("click", function () {
    let logout_request = new XMLHttpRequest();
    logout_request.open("POST", "/api/account/logout/", true);
    logout_request.withCredentials = true;
    logout_request.send();
